...
/** كرت عسل — نفس بنية كرت المكسرات (حد ذهبي، ProductImageFrame، حركة وتدرج) */
export default function HoneyCard({ product }: { product: HoneyProduct }) {
  ...
  return (
    <Link href={`/products/${product.id}`} className="block">
      ...
      >
        <div className="pointer-events-none absolute -inset-px rounded-[1.75rem] opacity-[0.35] blur-xl md:opacity-[0.5] md:blur-2xl">
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_82%,rgba(255,210,120,0.2),transparent_58%)]" />
        </div>
        ...
        <div className="pointer-events-none absolute -inset-10 hidden opacity-0 transition-opacity duration-500 group-hover:opacity-100 md:block">
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_44%_22%,rgba(217,164,65,0.2),transparent_52%),radial-gradient(circle_at_68%_84%,rgba(255,236,210,0.2),transparent_58%)] blur-2xl" />
        </div>
          ...
          <div className="h-[200px] overflow-hidden rounded-[1.45rem] sm:h-[250px] lg:h-[320px]">
            {hasImage ? (
              ...
              <div className="relative h-full w-full">
                <div className="absolute inset-0 bg-[radial-gradient(circle_at_35%_25%,rgba(217,164,65,0.22),transparent_58%),radial-gradient(circle_at_75%_78%,rgba(255,220,170,0.09),transparent_58%)]" />
                <div className="absolute inset-0 bg-[linear-gradient(145deg,rgba(14,12,10,0.96),rgba(22,17,11,0.9),rgba(8,7,6,0.95))]" />
                <div className="absolute inset-0 grid place-items-center p-6">
                  <div className="w-full max-w-xs rounded-[1.35rem] border border-[rgba(217,164,65,0.28)] bg-[rgba(11,9,7,0.78)] p-6 text-center backdrop-blur-sm md:bg-[rgba(11,9,7,0.65)] md:backdrop-blur-md">
                    <motion.div
                    ...
                    >
                      <Droplet className="size-4 text-[#D9A441]" aria-hidden />
        ...
        <div className="relative p-6">
          <h3 className="lux-card-title text-xl font-semibold tracking-tight text-[#F4E4B8]">
            {product.name}
          </h3>
